import { ITrelloNotification } from "trello-notofication-api";
import { Page } from "../../../app/page/page.js"
import { DateDistance } from "../../../Lib/DateDistance.js";

export class csTrelloNotification extends HTMLElement {
    container!: ShadowRoot;
    item !: ITrelloNotification;
    InitalizeComponent(): any {
        // Create a shadow root
        this.container = this.attachShadow({ mode: 'open' });
    }
    connectedCallback() {
        if (!this.item && !this.hasAttribute("data-src")) return;
        this.item = JSON.parse(this.getAttribute("data-src") as string) as ITrelloNotification;
        ...
        if (this.item.type == "changeCard") {
            ...
            if (this.item.data.old) {
                Object.keys(this.item.data.old).forEach(key => {
                    ...
                    var cc = this.item.data.card as { [x: string]: any };
                    var cold = this.item.data.old as { [x: string]: any };
                    if (key == "due") {
