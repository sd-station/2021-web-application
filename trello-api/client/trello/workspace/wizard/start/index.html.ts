...
form.btnSubmit.addEventListener("click", async _ => {
    ...
    var data = {} as { [x: string]: string }


    if (form.txtWorkspaceName.Text.length > 0) data.displayName = form.txtWorkspaceName.Text.trim();
    if (form.txtWorkspaceId.Text.length > 0) data.name = form.txtWorkspaceId.Text.trim();
    if (form.txtWorkspaceDesc.Text.length > 0) data.desc = form.txtWorkspaceDesc.Text.trim();
    if (form.txtWorkspaceWebsite.Text.length > 0) data.website = form.txtWorkspaceWebsite.Text.trim();
    ...
    if (loader.ResponseType == "item") {
        ...
        if (Page.Level == 3) { Page.Navigate("workspace/wizard/step-2"); }
        else { Page.Navigate("workspace/view") }
    }
})
