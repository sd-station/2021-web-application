...
import { csAppTimeline } from "../../../ui/time-line/app-timeline.js";
import { TrelloCardApi } from "../../api/card-api.js";
import { CardUpdateApi } from "../../api/card-update.api.js";
import { csCardColorPicker } from "../define/card-colorpicker.js";
...
var Timeline = document.querySelector("app-timeline") as csAppTimeline;
...
if (data.HasItem) {
   ...
   var itm = data.ItemOf<ITrelloCard>();
   ...
   if (itm.start) info["start"] = new Date(itm.start).toLocaleString();
   ...
   if (!User.trello.list.selected) User.trello.list.id = itm.idList;
...
document.querySelectorAll("[data-action]").forEach(da => {
   da.addEventListener("click", async _ => {
      ...
      switch (command) {
         case "edit-cover":
            var colorpicker = document.createElement("card-colorpicker") as csCardColorPicker;
