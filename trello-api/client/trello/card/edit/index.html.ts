...
var cmbBoard = form.txtBoardName.Element as HTMLSelectElement;
var cmbList = form.txtListName.Element as HTMLSelectElement;
//>> Assign Data

window.addEventListener('message', async (event) => {
    var pass = event.data as {
        command: string;
        value: string;
    }
    ...
    if (pass.command == "set-data" && pass.value == "manual") {
        ...
        if (boardAdp.HasList) {
            var boards = boardAdp.ToListOf<ITrelloBoard>();
...
form.btnSubmit.addEventListener("click", async _ => {
    ...
    if (x1.HasItem) {
        var itm = x1.ItemOf<ITrelloCard>();
        var dtav = (form.txtDueDate.Element as HTMLInputElement).valueAsNumber + new Date().getTimezoneOffset() * 60 * 1000;
