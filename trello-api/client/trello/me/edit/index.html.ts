import { ITrelloMemberAPI } from "trello-member-api";
...
if (data.HasItem) {
    var m = data.ItemOf<ITrelloMemberAPI>();
    //>> Assign Data
    ...
    form.btnSubmit.addEventListener("click", async _ => {
        ...
        var options = {} as { [x: string]: string };
        if (form.txtFullName.Text != m.fullName) options["fullName"] = form.txtFullName.Text;
        if (form.txtUsername.Text != m.username) options["username"] = form.txtUsername.Text;
        if (form.txtInital.Text != m.initials) options["initials"] = form.txtInital.Text;
        if (form.txtbio.Text != m.bio) options["bio"] = form.txtbio.Text;
