...
form.InputFile.Element.addEventListener("change", _ => {
    const myFiles = (form.InputFile.Element as HTMLInputElement).files;
    if (!myFiles || myFiles.length <= 0) return;
    ...
    XS.AfterSuccess = (number, resitem: any) => {
        if (number == 200) {
...
function SendFailer(number: number, resitem: any) {
