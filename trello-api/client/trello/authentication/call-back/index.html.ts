...
let testtoken = "";
let itm: ITrelloMemberAPI;


function get_gravatar(id: string, avatarHash: string, size: number) {
    var size = size || 80;
...
async function init() {
    ...
    itm = dataloader.data as ITrelloMemberAPI;


    var Nmit = new ComponentProvider("identity-card");
    Nmit.Item = itm;
    if (itm.avatarHash) Nmit.Manual["profile-photo"] = get_gravatar(itm.id, itm.avatarHash, 170);
    else Nmit.Manual["profile-photo"] = "/assets/icons/social/trello.svg";
    ...
    document.querySelector("identify-card")!.innerHTML = content;
     ...
     } else console.error("token", TokenAdaptor.RawText);
