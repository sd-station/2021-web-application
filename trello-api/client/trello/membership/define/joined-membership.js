...
export class csJoinedMembership extends HTMLElement {
    ...
    async Display_Data() {
        ...
        if (User.trello.member.id == this.item.id)
            data.name += " (you)";
        ...
        if (usr.HasItem) {
            var membership = usr.ItemOf();
            if (membership.avatarUrl) {
                var img = this.container.querySelector("#profile-image");
...
//# sourceMappingURL=joined-membership.js.map